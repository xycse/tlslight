...
{
	/*TODO: Need to populate this function its needed to prevent session hijacking */
	return 1;
}
...
static int tls_ca_update(SSL_CTX *ctx)
{
	...
	if (!SSL_CTX_load_verify_locations(ctx, cafile /* client can do with this arg as NULL */, "/etc/ssl/certs"))
	{
		while ((error = ERR_get_error()))
	...
	/* we can do without this block too */
	/* block starts */
	...
	/* TODO: need to check cookie related code carefully, need to readup on its significance and how to implement it */
	return 0;
}
...
int main(void)
{
	...
	/* server.crt is needed; btw its client cert, I've used the word server.crt here due to lazyness */
	...
	char *servername = "142.250.192.68";
